...
# --- 2. ENTRADAS DE PROYECTO ---
with st.sidebar:
    st.header("📋 Identificación")
    c_tag = st.text_input("Nombre/Tag del Circuito", "C-01")
    c_origen = st.text_input("Origen (De:)", "Tablero General")
    c_destino = st.text_input("Destino (A:)", "Carga Final")
...
# Tubería
area_tot = (AREAS_MM2.get(cal_final, 15) * (fases + 1)) # Fases + Neutro/Tierra
tubo = next((t for t, cap in sorted(CONDUIT_40.items(), key=lambda x: x[1]) if area_tot <= cap), "Revisar")

# --- 4. RESULTADOS ---
st.info(f"Circuito: **{c_tag}** | {c_origen} ➔ {c_destino}")
c1, c2, c3 = st.columns(3)
with c1: st.success(f"**Conductor:** {cal_final}")
with c2: st.metric("Caída de Tensión", f"{round(vd_final,2)} %")
with c3: st.info(f"**Tubería:** {tubo}")
...
1. DATOS DE DISEÑO:
- Carga: {val_p} {tipo_p} (Calculado: {pkva:.2f} kVA)
- Voltaje: {v} V | Fases: {fases} | FP: {fp}
- Distancia: {dist} m

2. RESULTADOS DE CORRIENTE:
- I_nominal: {i_nom:.2f} A
- I_diseño (125%): {i_dis:.2f} A

3. ESPECIFICACIÓN TÉCNICA:
- CONDUCTOR SELECCIONADO: {cal_final} Cu 75°C
- CAÍDA DE TENSIÓN FINAL: {vd_final:.2f} %
- CANALIZACIÓN SUGERIDA: {tubo} Conduit Pared Gruesa

4. NOTAS:
- Cálculo basado en NOM-001-SEDE / NEC.
- Se garantiza caída de tensión < 3% por diseño.
-----------------------------------------------------------
